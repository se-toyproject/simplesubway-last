...
"1": ["소요산", "동두천", "보산", "동두천중앙", "지행", "덕정", "덕계", "양주", "녹양", 
...
"3": ["대화", "주엽", "정발산", "마두", "백석", "대곡", "화정", "원당", "원흥", "삼송", 
...
"4": ["당고개", "상계", "노원", "창동", "쌍문", "수유", "미아", "미아사거리", "길음", 
    ...
    "오이도"],
"5": ["방화", "개화산", "김포공항", "송정", "마곡", "발산", "우장산", "화곡", "까치산", 
    ...
    "방이", "오금", "개롱", "거여", "마천"],
"6": ["응암순환(상선)", "역촌", "불광", "독바위", "연신내", "구산", "새절(신사)", 
    ...
    "봉화산", "신내"],
"7": ["장암", "도봉산", "수락산", "마들", "노원", "중계", "하계", "공릉(서울산업대입구)", 
...
"8": ["별내", "다산", "남양주양정", "별내별가람", "초지",  "암사", "천호(풍납토성)", "강동구청", 
    "몽촌토성(평화의문)", "잠실", "석촌", "송파", "가락시장", "문정", "장지", "복정", "남위례", "산성", 
    "남한산성입구(성남법원,검찰청)", "단대오거리", "신흥", "수진", "모란"],
"9": ["개화", "김포공항", "공항시장", "신방화", "마곡나루", "양천향교", "가양", 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    lineSelect.addEventListener('change', function() {
        ...
        stationSelect.innerHTML = '<option value="">역을 선택해주세요</option>';

        if (selectedLine in stationsByLine) {
            stationsByLine[selectedLine].forEach(function(station) {
                const option = document.createElement('option');
                option.value = station;
                option.textContent = station;
                stationSelect.appendChild(option);
            });
            messageDiv.textContent = `${selectedLine}호선이 선택되었습니다. 출발역을 선택해주세요.`;
        } else {
            messageDiv.textContent = '호선을 선택해주세요.';
        }
    });
    ...
    searchForm.addEventListener('submit', function(e) {
        ...
        if (selectedLine && selectedStation) {
            messageDiv.textContent = `${selectedLine}호선 ${selectedStation}역이 선택되었습니다.`;
        } else {
            messageDiv.textContent = '호선과 역을 모두 선택해주세요.';
        }
    });
});